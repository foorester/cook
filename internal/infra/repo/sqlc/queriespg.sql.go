...
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, name, description, recipe_id, qty, unit, created_at, updated_at
`
...
VALUES ($1, $2, $3, $4)
RETURNING id, name, description, book_id, created_at, updated_at
`
...
VALUES ($1, $2, $3, $4, $5)
RETURNING id, name, description, recipe_id, duration, created_at, updated_at
`
...
func (q *Queries) SelectAllBooks(ctx context.Context, ownerID pgtype.UUID) ([]Book, error) {
	rows, err := q.db.Query(ctx, selectAllBooks, ownerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type SelectIngredientsByRecipeIDRow struct {
	ID          pgtype.UUID
	Name        string
	Description string
	RecipeID    pgtype.UUID
...
func (q *Queries) SelectIngredientsByRecipeID(ctx context.Context, recipeID pgtype.UUID) ([]SelectIngredientsByRecipeIDRow, error) {
	rows, err := q.db.Query(ctx, selectIngredientsByRecipeID, recipeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SelectIngredientsByRecipeIDRow
	for rows.Next() {
		var i SelectIngredientsByRecipeIDRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Description,
...
type SelectStepsByRecipeIDRow struct {
	ID          pgtype.UUID
	Name        string
	Description string
	RecipeID    pgtype.UUID
...
func (q *Queries) SelectStepsByRecipeID(ctx context.Context, recipeID pgtype.UUID) ([]SelectStepsByRecipeIDRow, error) {
	rows, err := q.db.Query(ctx, selectStepsByRecipeID, recipeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SelectStepsByRecipeIDRow
	for rows.Next() {
		var i SelectStepsByRecipeIDRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Description,
...
func (q *Queries) SelectUserByID(ctx context.Context, id pgtype.UUID) (SelectUserByIDRow, error) {
