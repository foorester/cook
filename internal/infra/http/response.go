...
func (h *CookHandler) handleSuccess(w http.ResponseWriter, payload interface{}, count, pages int, msg ...string) {
	var m string
	if len(msg) > 0 {
		m = msg[0]
	}
...
func (h *CookHandler) handleError(w http.ResponseWriter, handlerError error) {
	response := APIResponse{
		Success: false,
